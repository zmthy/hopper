...
var keywords, puncSymbols, unicode, util;

unicode = require("./unicode");
util = require("./util");

keywords =
  ["class", "def", "dialect", "false", "import", "inherits", "is", "method",
    "object", "outer", "return", "self", "super", "true", "type", "var"];
...
function Lexer(text, error) {
  ...
  // new this.Token(type : String, value : String, indent : Number = undefined)
  this.Token = function (type, value, indent) {
    this.type  = type;
    this.value = value;
...
Lexer.prototype.nextToken = function (interpolating) {
  ...
  function testNewline(j) {
    ...
    if (d === "\r") {
      if (text[j + 1] !== "\n") {
        lexer.error("Invalid Unicode character \\r without corresponding \\n");
      }
  ...
  function handleNewline() {
    ...
    return update(new lexer.Token("newline", c, spaces));
  }
  ...
  if (i >= l) {
    return new this.Token("eot", "end of input");
  }

  // Pick which token to create based on the current character.
  if (c === '"' || interpolating) {
    token = new this.Token("string", "");
    escaped = false;
  } else if (unicode.isLetter(c)) {
    token = new this.Token("identifier", c);
  } else if (unicode.isNumber(c)) {
    token = new this.Token("number", c);
    dot = false;
  } else if (isSymbol(c) || (c === "." && text[i + 1] === ".")) {
    token = new this.Token("symbol", c);
  } else {
    if (unicode.isPunctuation(c)) {
      return increment(new this.Token("punctuation", c));
    }
    ...
    if (c === "\t") {
      error("Invalid Unicode character \\t: tabs are banned");
    }
  ...
  // Unless the current token ends, loop one past the end of the text. The
  // character in the final pass will be null, which causes any token parsing to
  // end and the token to be tidied up.
  while (i <= l) {
    c = i !== l ? text[i] : null;
    ...
    if (token.type === "identifier") {
      ...
      if (unicode.isNumber(c) ||
        ...
        if (/[eExX]/.test(c)) {
          error("Dangling modifier on number " + token.value);
        }
      ...
      if (escaped) {
        if (/["\\\\{}]/.test(c)) { //"]) {
          ...
          if (c.length < 4 || !/[0-9a-fA-F]/.test(c)) {
            error("Invalid Unicode reference value " + c);
          }

          token.value += String.fromCharCode("0x" + c);
          i += 4;
        } else {
          error("Unrecognised escape character \\" + c);
        }
      } else {
        if (c === '"') {
  ...
  if (token.type === "string") {
    error("Unclosed string \"" + unicode.escape(token.value));
  }
