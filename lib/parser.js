...
var ast, error, lexer, lookahead, task, tokens, util;
  ...
  "parsers": function (name) {
    ...
    function run(test, failure) {
      return function () {
        ...
        then = (result) => after ? after.call(this, result) : result;
...
Parser.prototype.module = function () {
  return this.lone("dialect").then((dialect) => {
    return this.any("import").then((imports) => {
      if (dialect !== null) {
    ...
    return this.objectBody().then((body) => {
...
Parser.prototype.def = lookahead.keyword("def", function () {
  ...
  return this.on("symbol", ":", () => {
    return this.expression();
  }).then((pattern) => {
    return this.lone("annotations").then((annotations) => {
      this.inDef = false;
      ...
      return this.expression().then((value) => {
        this.newline();
...
Parser.prototype["var"] = lookahead.keyword("var", function () {
  ...
  return this.on("symbol", ":", () => {
    ...
    return this.lone("annotations", true).then((annotations) => {
      if (this.test("symbol", "=")) {
...
Parser.prototype.declOrLiteral = lookahead.keyword("type", function () {
  return this.attempt(function () {
    ...
    return this.on("symbol", "<", () => {
      return this.commas("identifier").then((generics) => {
        this.symbol(">");
        return generics;
      });
    }).then((generics) => {
      return this.lone("annotations").then((annotations) => {
        ...
        return this.lone("typeBraces").then((type) => {
...
Parser.prototype.typeBraces = lookahead.punctuation("{", function () {
  return this.braces(function (token) {
    return this.any("typeBody").then((body) => {
...
Parser.prototype.methodRest = lookahead.name(function (parser, make) {
  return this.signature().then((signature) => {
    return this.lone("annotations").then((annotations) => {
      ...
      return this.braces(function () {
        return this.one(parser).then((result) => {
...
Parser.prototype.signature = lookahead.name(function () {
  return this.signaturePartFirst().then((first) => {
    return this.task(function () {
      if (first.parameters.length === 0 || first.name.isOperator) {
        return [first];
      }

      return this.any("signaturePartRest").then((rest) => {
        rest.unshift(first);
        return rest;
      });
    }).then((parts) => {
...
Parser.prototype.signaturePartFirst = lookahead.name(function () {
  return this.lone("operator").then((operator) => {
    if (operator === null) {
    ...
    if (!name.isOperator) {
      if (name.value === "prefix") {
        return this.on("operator", (operator) => {
      ...
      if (this.test("symbol", ":=")) {
        ...
        return this.parentheses(this.parameter).then((parameter) => {
...
Parser.prototype.signaturePartPost = function (name, first) {
  return this.task(function () {
    ...
    return this[first ? "lone" : "one"]("parentheses", function () {
      if (name.isOperator) {
        return this.parameter().then((parameter) => {
          return [parameter];
        });
      }
...
Parser.prototype.vararg = lookahead.symbol("*", function () {
  ...
  return this.parameterName().then((name) => {
    return this.parameterType().then((type) => {
...
Parser.prototype.commas = function (parser) {
  ...
  return this.one(parser).then((first) => {
...
Parser.prototype.inherits = lookahead.keyword("inherits", function () {
  ...
  return this.expression().then((request) => {
...
Parser.prototype["return"] = lookahead.keyword("return", function () {
  ...
  return this.lone("expression").then((expression) => {
    this.newline();
...
Parser.prototype.expression = lookahead.parsers("expression",
  "preBinaryOperator", function (expression) {
    ...
    function operators(lhs, lop, rhs) {
      return this.on("operator", (rop) => {
        return this.preBinaryOperator().then((pre) => {
    ...
    return this[which]("operator").then((op) => {
      return op && this.preBinaryOperator().then((pre) => {
...
Parser.prototype.preBinaryOperator = lookahead.parsers("expression", "receiver",
  function (expression) {
    ...
    var requests = (receiver) => {
      return this.on("dotRequest", (signature) => {
        return requests(new ast.QualifiedRequest(receiver, signature));
      }).then((request) => {
        return request || receiver;
      });
    };

    return requests(expression);
  });
...
Parser.prototype.prefixOperator = lookahead.operator(function () {
  ...
  return this.receiver().then((receiver) => {
...
Parser.prototype.unqualifiedRequest = lookahead.identifier(function () {
  return this.requestSignature().then((signature) => {
...
Parser.prototype.requestSignature = lookahead.identifier(function () {
  return this.requestPart(false).then((first) => {
    if (first.arguments.length === 0) {
      return [first];
    }

    return this.any("requestPart", true).then((parts) => {
      parts.unshift(first);
      return parts;
    });
  });
  ...
  return this.task(function () {
    ...
    if (this.test("symbol", "<") && !this.peek().spaced) {
      ...
      return this.attempt(function () {
        ...
        return this.commas("expression").then((types) => {
          var after, next;
    ...
    return this.on(this.isStrict ? "strictLiteral" : "literal", (arg) => {
      ...
      if (!required && !this.isStrict && args === null) {
        return this.on("symbol", ":=", () => {
          ...
          return this.expression().then((expression) => {
            return [expression];
          });
        });
      }
...
Parser.prototype.binaryRequestSignature = lookahead.operator(function () {
  ...
  return this.expression().then((rhs) => {
    ...
    return this.attempt(function () {
      return this.task(function () {
        if (!this.test("identifier") && !this.test("punctuation", "_")) {
          return this.expression().then((params) => {
            return [
      ...
      return this.any("statement").then((body) => {
...
Parser.prototype.objectBody = function () {
  return this.lone("inherits").then((inherits) => {
    return this.any("statementOrMethod").then((body) => {
      if (inherits !== null) {
...
Parser.prototype.any = function () {
  ...
  function any(results) {
    if (this.test.apply(this, args)) {
      return this.one.apply(this, args).then((result) => {
...
Parser.prototype.many = function () {
  return this.one.apply(this, arguments).then((result) => {
    return this.any.apply(this, arguments).then((results) => {
...
Parser.prototype.on = function () {
  ...
  if (this.test.apply(this, args)) {
    return this.one.apply(this, args).then((result) => {
      return arguments[l].call(this, result);
    });
  }
...
Parser.prototype.strict = function (func, isStrict) {
  ...
  return this.resolve(func.call(this)).then((result) => {
    this.isStrict = state;
...
function runParser(code) {
  ...
  try {
    ...
    return parser.module().then((module) => {
      do {
        token = parser.poll();
...
function parse(code, path) {
  return runParser(code).then(null, (reason) => {
    reason.module = path;
    throw reason;
  });
}
