...
Parser.prototype.methodRest = lookahead.name(function (parser, make) {
  var annotations, signature;

  signature = this.signature();
  annotations = this.lone("annotations") || [];

  return this.braces(function () {
    return make.call(this, signature, annotations, this.one(parser));
  });
});

Parser.prototype.signature = lookahead.name(function () {
  var first, parts, pattern;

  first = this.signaturePartFirst();
  parts = first.parameters.length === 0 || first.name.isOperator ?
...
Parser.prototype.requestPart = lookahead.identifier(function (required) {
  ...
  this.on(this.isStrict ? "strictLiteral" : "literal", function (arg) {
    args = [arg];
  ...
  if (!required && !this.isStrict && args === undefined) {
    this.on("symbol", ":=", function () {
      name.value += " :=";
  ...
  return this.braces(function (token) {
    ...
    parameters = this.attempt(function () {
      ...
      if (!this.test("identifier") && !this.test("punctuation", "_")) {
        ...
        params = [new ast.Parameter(new ast.Identifier("_", false, params),
          params, false, params)];
      } else {
        params = this.commas("parameter");
      }
