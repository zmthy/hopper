...
Parser.prototype.newline = lookahead.newline(function () {
  ...
  if (token.value !== "}") {
    if (token.value === ";") {
      ...
      if (token === null || token.constructor !== tokens.Newline) {
        error.raise(token, "Unexpected use of " + token);
      }
...
Parser.prototype.wrapped = function (o, c, f) {
  ...
  if (!this.test("punctuation", c)) {
    ...
    if (token.constructor === tokens.EndOfInput) {
      error.raise(token, "Unexpected " + token);
    }

    error.raise(token, "Cannot use " + token + " here");
  }
...
Parser.prototype.requestPart = lookahead.identifier(function (required) {
  ...
  if (this.test("symbol", "<") && !this.peek().spaced) {
    ...
    generics = this.attempt(function () {
      ...
      after = this.peek().constructor;

      if (after === tokens.Identifier || after === tokens.Keyword) {
...
function parse(code) {
  ...
  if (token.constructor !== tokens.EndOfInput) {
    error.raise(token, "Unexpected leftover token " + token);
  }
