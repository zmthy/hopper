...
function precedence(lhs, rhs) {
  ...
  if (!isMathOperator(left) || !isMathOperator(right)) {
    error(left, "Mismatched operators " + left + " and " + right);
  }
  ...
  if (token.value !== "}") {
    if (token.value === ";") {
      ...
      if (indent !== this.indent && this.peek().value !== "}") {
        error(indent, "Indent must match previous line");
      }
    }
  }
});
...
Parser.prototype.braces = lookahead.punctuation("{", function (f) {
  ...
  result = this.wrapped("{", "}", function (token) {
    ...
    if (next.type === "newline") {
      ...
      if (indent <= this.indent && this.peek().value !== "}") {
        error(indent, "Invalid indent following opening brace");
      }
...
Parser.prototype.preBinaryOperator = lookahead.parsers("expression",
  "object", "type", "unqualifiedRequest", "literal", "true", "false", "outer",
...
Parser.prototype.operator = lookahead.value("symbol", function (symbol) {
  ...
  var token = this.expect("symbol", "operator");
...
Parser.prototype.identifier = lookahead.identifier(function () {
  var token = this.expect("identifier");
...
Parser.prototype.error = function (type, token) {
  ...
  error(token, "Expected " + type + ", but found '" + token + "'");
};
...
function parse(code) {
  ...
  parser = new Parser(new lexer.Lexer(code, error));
  ...
  if (token.type !== "eot") {
    token(token, "Unexpected leftover token '" + token + "'");
  }
