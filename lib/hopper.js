...
var Task, fs, interpreter, parser, rt, util;
...
function defaultLoader(interpreter, path, callback) {
  ...
  fs.readFile(relative + ".grace", function (exists, code) {
    ...
    if (exists === null) {
      ...
      interpreter.module(path, code, callback);
    } else {
      try {
        result = require(relative);
      } catch (problem) {
        return (problem.code === "MODULE_NOT_FOUND" ?
            rt.UnresolvedModule.raiseForPath(rt.string(path)) :
            rt.InternalError.raiseFromPrimitiveError(problem))
          .then(null, callback);
      }
