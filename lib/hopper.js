...
function parseAndHandle(path, code) {
  return parser.parse(path, code).then(null, function (error) {
...
function makeInterpret(method, optionalPath, parse, onSuccess, onFailure) {
  return function (path, code, callback) {
    ...
    return (util.isArray(code) ? next(code) : parse(path, code).then(next))
...
function splitArgs(args) {
  ...
  return {
    interpreter: util.slice(args, required, len),
    method: util.slice(args, 0, required).concat([args[len]])
  };
}

function build(args) {
  function Build() {
    Interpreter.apply(this, args);
  }
...
function buildAndApply(method, args) {
  ...
  terp = build(args.interpreter);

  return terp[method].apply(terp, args.method);
}

exports.CheckResult = CheckResult;
exports.Interpreter = Interpreter;

// compile(path : Path = undefined, code : String, prelude : Object = <sys>,
//     moduleLoader : Function<Interpreter, Path, Callback<Object>> = <fs>,
//     callback : Callback<Object>)
//   Compile Grace code.
exports.compile = function () {
  var args, callback, terp;

  args = splitArgs(arguments);
  terp = build(args.interpreter);
  callback = args.method[args.method.length - 1];

  parseAndHandle.apply(null, args.method).then(function (nodes) {
    Task.each(nodes, function (node) {
