...
var Task, loader, fs, interpreter, parser, prelude, rt, util;
...
function parse(text, onError, onResult) {
  ...
  try {
    ...
    return (error instanceof parser.ParseError ?
        rt.ParseError.raiseMessage(rt.string(error.message)) :
        rt.InternalError.raiseMessage(rt.string(error.message)))
      .then(null, function (packet) {
        packet.object.stackTrace
          .push("(line " + error.line + ", column " + error.column + ")");
...
function Interpreter(scope, moduleLoader) {
  var self = this;
  ...
  this.prelude = Task.resolve(scope || prelude);
...
Interpreter.prototype.interpret = function (code, callback) {
  ...
  parse(code, callback, function (ast) {
    self.prelude.then(function () {
      self.interpreter.interpret(ast).callback(callback);
    }, callback);
  });
};
...
Interpreter.prototype.module = function (path, code, callback) {
  ...
  parse(code, function (packet) {
    var trace = packet.object.stackTrace;

    trace[0] = '"' + path + '" ' + trace[0];

    callback(packet);
  }, function (ast) {
    self.prelude.then(function () {
      self.interpreter.module(path, ast).callback(callback);
    }, callback);
  });
};
...
Interpreter.prototype.load = function (path, callback) {
  ...
  self.prelude.then(function () {
    self.interpreter.load(path).callback(callback);
  }, callback);
};
...
Interpreter.prototype.enter = function (callback) {
  ...
  self.prelude.then(function () {
    var object = self.interpreter.enter();
...
// interpret(code : String, prelude : Object = <sys>,
//     moduleLoader : Function<Interpreter, Path, Callback<Object>> = <fs>,
