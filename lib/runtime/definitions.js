...
Unknown.match = rt.method("match()", 1, function () {
  return bool(true);
});
...
Action.assert = rt.method("assert()", 1, function () {
  return done;
});
...
exports.isInternalError = function (error) {
  ...
  if (isGraceExceptionPacket(error)) {
    exceptions.InternalError.match(error).now(function (match) {
      return match.orElse(rt.block(0, function () {
        isInternal = false;
      })).now();
    });
  }
...
exports.try_catch =
  rt.method("try() catch()", [[1, 1], [1, 1]], function (fst, snd) {
