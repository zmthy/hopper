...
var Bool, Do, Foreign, Node, None, Order, Pattern, Sequence, Unknown,
  ast, defs, rt, type, util;
...
None = defs.pattern("None", defs.failure);

exports.None = None;

None["<:"] = rt.method("<:", 1, function (pattern) {
  return Pattern.assert(pattern).then(function () {
    return defs.bool(true);
  });
});

Unknown = defs.pattern("Unknown", function (object) {
  return defs.success(object);
});

Unknown["<:"] = rt.method("<:", 1, function (pattern) {
  return Pattern.assert(pattern).then(function () {
    return defs.bool(true);
  });
});
