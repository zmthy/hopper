...
RErr.refine_defaultMessage([str("Unresolved Request")],
  ...
  .now(function (UnresolvedRequest) {
    var post, preConf, preMethod, postQualified, preQualified, preVar;
    ...
    addRaise(UnresolvedRequest, "ForName", 1, function (name) {
      ...
      return rt.String.cast(name).then(function (name) {
  ...
  .now(function (AssertionFailure) {
    ...
    addRaise(AssertionFailure, "ForValue() againstPattern()", [1, 1],
      function (value, pattern) {
        ...
        return asString(value[0]).then(function (value) {
    ...
    addRaise(AssertionFailure, "ForValue() againstType() missing()", [1, 1, 1],
      function (value, pattern, signature) {
        var self = this;

        return asString(value[0]).then(function (value) {
          return join(open, value, miss, signature[0], post, pattern[0], close)
            .then(function (message) {
              return self.raise(message);
            });
        });
...
LErr.refine(str("Failed Search")).now(function (FailedSearch) {
  var pre = str("Cound not find the object «");
...
LErr.refine_defaultMessage([str("Out Of Bounds")],
  ...
  .now(function (OutOfBounds) {
    ...
    addRaise(OutOfBounds, "ForIndex()", 1, function (index) {
      ...
      return defs.Number.cast(index).then(function (index) {
...
LErr.refine(str("Not A Number")).now(function (NotANumber) {
  ...
  addRaise(NotANumber, "ForParse()", 1, function (string) {
    ...
    return rt.String.cast(string).then(function (string) {
      return asString(string).then(function (string) {
        return join(preParse, string, postParse).then(function (message) {
          return self.raise(message);
        });
      });
    });
...
Exception.refine(str("Checker Failure")).now(function (CheckerFailure) {
  CheckerFailure.object.Packet.prototype.nodeOrIfAbsent =
    rt.method("nodeOrIfAbsent", 1, function (action) {
      return rt.Action.cast(action).then(function (action) {
        return action.apply();
      });
    });
