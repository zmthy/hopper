...
LErr.refine_defaultMessage([ str("Assertion Failure") ],
  ...
  .now(function (AssertionFailure) {
    ...
    addRaise(AssertionFailure, "ForClause() ofMethod()", [ 1, 1 ],
      function (clause, name) {
        var self = this;

        return defs.String.assert(clause[0]).then(function () {
          return join(preClause, clause[0], midClause, name[0], close)
            .then(function (message) {
              return self.raise(message);
            });
        });
