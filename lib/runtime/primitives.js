...
addMethod(Type, "match()", 1, function (value) {
  ...
  names = this.object.names;

  for (i = 0, l = names.length; i < l; i += 1) {
    if (value[util.uglify(names[i])] === undefined) {
      return defs.failure(value);
    }
  }
...
addMethod(List, "do()", [[1, 1]], function (T, action) {
  var self = this;
  ...
  return this.pattern().then(function (pattern) {
    return new Task(function (resolve, reject) {
      defs.List.match(list).then(function (isList) {
        return isList.ifTrue_ifFalse([defs.block(0, function () {
