...
addMethod(InternalArray, "fold()", [[1, 1]], function (pattern, fold) {
  ...
  return defs.Function2.cast(fold).then(function (fold) {
    return self.asPrimitiveString().then(function (string) {
      var value;
      ...
      return Task.each(string.slice(1), function (element) {
        return fold.apply(value, element).then(function (result) {
          return pattern.assert(result).then(function () {
            value = result;
            return rt.done;
...
addMethod(InternalArray, "fold()", [[1, 1]], function (pattern, fold) {
  ...
  return defs.Function2.cast(fold).then(function (fold) {
    return Task.each(elements.slice(1), function (element) {
      return fold.apply(value, element).then(function (result) {
        return pattern.assert(result).then(function () {
          value = result;
          return rt.done;
...
addConstructor(Exception, "refine()", 1, function (inherit, name) {
  return Task.resolve(this._refine(name, null, inherit));
});

addConstructor(Exception, "refine() defaultMessage()", [1, 1],
  function (inherit, name, defMessage) {
    return Task.resolve(this._refine(name, defMessage, inherit));
  });
