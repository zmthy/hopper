...
function toNumber(number) {
  return defs.Number.cast(number).then(function (number) {
    return number.asPrimitiveNumber();
  });
}

function toString(string) {
  return defs.String.cast(string).then(function (string) {
  ...
  asString(this).now(function (value) {
    ...
    reason.exception().then(function (exception) {
      return exception.name().then(function (name) {
        return toString(name).then(function (name) {
          error.name = name;
        });
      });
    }).then(function () {
      return reason.message().then(function (message) {
        return toString(message).then(function (message) {
...
function binaryOp(func, type) {
  return function (rhs) {
    ...
    return defs[type].cast(rhs).then(function (rhs) {
...
addMethod(GraceString, "fold() startingWith()", [[1, 1], 1],
  function (fold, value) {
    ...
    return defs.Function2.cast(fold[1]).then(function (fold) {
...
addMethod(GraceString, "substringFrom() to()", [1, 1], function (from, to) {
  var self = this;

  return toNumber(from[0]).then(function (from) {
    return toNumber(to[0]).then(function (to) {
      return self.asPrimitiveString().then(function (self) {
        if (from < 1 || from > self.length + 1) {
          return defs.OutOfBounds.raiseForIndex(defs.number(from));
        }

        if (to < 1 || to > self.length + 1) {
          return defs.OutOfBounds.raiseForIndex(defs.number(to));
        }

        return defs.string(self.substring(from - 1, to - 1));
      });
    });
  });
});

addMethod(GraceString, "substringFrom() size()", [1, 1], function (from, size) {
  var self = this;

  return toNumber(from[0]).then(function (from) {
    return toNumber(size[0]).then(function (size) {
      return self.asPrimitiveString().then(function (self) {
        var to = from + size;
...
addMethod(GraceString, "replace() with()", [1, 1], function (from, to) {
  var self = this;

  return toString(from[0]).then(function (from) {
    return toString(to[0]).then(function (to) {
      return self.asPrimitiveString().then(function (self) {
        return defs.string(self.replace(from, to));
      });
    });
  });
});

addMethod(GraceString, "startsWith()", 1, function (prefix) {
  var self = this;

  return toString(prefix).then(function (prefix) {
    return self.asPrimitiveString().then(function (self) {
      ...
      return self.lastIndexOf(prefix, index) === 0;
    });
  });
});

addMethod(GraceString, "endsWith()", 1, function (suffix) {
  var self = this;

  return toString(suffix).then(function (suffix) {
    return self.asPrimitiveString().then(function (self) {
      var index = self.length - suffix.length;

      if (index < 0) {
        return defs.boolean(false);
      }

      return self.indexOf(suffix, index) === index;
    });
  });
});
...
function addIndexOfs(forwards) {
  ...
  defaultStart = forwards ? function () {
    ...
    return string.asPrimitiveString().then(function (string) {
      return defs.number(string.length);
    });
  };

  addMethod(GraceString, method + "() startingAt() ifAbsent()", [1, 1, [1, 1]],
    function (search, from, ifAbsent) {
      var self = this;

      return toString(search[0]).then(function (search) {
        return toNumber(from[0]).then(function (from) {
          return defs.Action.cast(ifAbsent[1]).then(function (absent) {
            return self.asPrimitiveString().then(function (self) {
              var index;

              if (from < 0 || from > self.length ||
                  (from === 0 && self.length !== 0)) {
                return defs.OutOfBounds.raiseForIndex(defs.number(from));
              }

              index = self[method](search, from - 1);

              if (index < 0) {
                return absent.apply().then(function (result) {
                  return ifAbsent[0].assert(result).then(function () {
                    return result;
                  });
                });
              }
  ...
  addMethod(GraceString, method + "()", 1, function (search) {
    ...
    return defaultStart(self).then(function (from) {
      return self[name]([search], [from], [defs.block(0, function () {
...
addMethod(Sequence, "++", 1, function (rhs) {
  ...
  return defs.Do.cast(rhs).then(function (rhs) {
    return getElements(rhs).then(function (rhs) {
      return defs.sequence(elements.concat(rhs));
    });
  });
});

addMethod(Sequence, "sliceFrom() to()", [1, 1], function (from, to) {
  var elements = this.object.elements;

  return toNumber(from).then(function (from) {
    if (from < 1 || from > elements.length + 1) {
      return defs.OutOfBounds.raiseForIndex(defs.number(from));
    }

    return toNumber(to).then(function (to) {
...
addMethod(Sequence, "asString", 0, function () {
  ...
  return self.at(defs.number(1)).then(function (head) {
    ...
    return self.sliceFrom(defs.number(2)).then(function (rest) {
      return rest.fold_startingWith([rt.block(2, function (s, e) {
        return s["++"](defs.string(", ")).then(function (s) {
          return rt.apply(e, "asString").then(function (e) {
            return s["++"](e);
          });
        });
      })], [head]);
...
addMethod(Sequence, "internalSplice()", rt.gte(2), function (index, amount) {
  var additions, elements;

  elements = this.object.elements;
  additions = util.slice(arguments, 2);

  return toNumber(index).then(function (index) {
    return toNumber(amount).then(function (amount) {
      return elements
...
function Trace(name, object, location) {
  ...
  this.receiverOrIfAbsent = rt.method("receiverOrIfAbsent()", [[1, 1]],
    function (absent) {
      var pattern = absent[0];

      return defs.Action.cast(absent[1]).then(function (absent) {
        if (object === null) {
          return absent.apply().then(function (result) {
            return pattern.assert(result).then(function () {
              return result;
  ...
  function fromLocation(name, type, prop) {
    self[name] = rt.method(name, 0,
      location === null || location[prop] === null ? function () {
        return defs.NoSuchValue
          .raiseForName_inObject([defs.string(name)], [self]);
      } : function () {
        return defs[type](location[prop]);
      });

    self[name + "OrIfAbsent"] = rt.method(name + "OrIfAbsent", [[1, 1]],
      function (absent) {
        var pattern = absent[0];
