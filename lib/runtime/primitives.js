...
function List(elements) {
  ...
  this.at = rt.newMethod("at()", 1, function (num) {
...
function ExceptionPacket(exception, message) {
  if (message === undefined) {
    this.asString = rt.newMethod("asString", function () {
  ...
  this.exception = rt.newMethod("exception", 0, function () {
    return exception;
  });

  this.message = rt.newMethod("message", 0, function () {
    return message;
  });
...
ExceptionPacket.prototype.raise.isUntraced = true;
...
function Exception(name, Packet) {
  this.name = rt.newMethod("name", 0, function () {
    return name;
  });

  this.raise = rt.newMethod("raise", function () {
    throw new Packet(this);
  });

  this.raise.isUntraced = true;

  this.raiseMessage = rt.newMethod("raiseMessage()", 1, function (message) {
    throw new Packet(this, message);
  });

  this.raiseMessage.isUntraced = true;

  this.refine = rt.newMethod("refine()", 1, function (name) {
    ...
    return Task.resolve(new Exception(name, ChildPacket));
  });

  this.refine_withDefaultMessage =
    rt.newMethod("refine() withDefaultMessage()", [1, 1],
      function (name, defMessage) {
        ...
        return Task.resolve(new Exception(name[0], ChildPacket));
      });

  this.match = rt.newMethod("match()", 1, function (value) {
    return Task.resolve(defs.match(value instanceof Packet, value));
  });
}
