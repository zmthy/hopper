...
Interpreter.prototype.interpret = function (nodes) {
  ...
  return this.imports(nodes).then(function () {
    ...
    return this.each(nodes, function (node) {
      ...
      if (constructor === ast.Class || constructor === ast.Let) {
        return this.putDecl(node);
      }
    });
  }).then(function () {
    ...
    return this.each(nodes, function (node) {
      // Imports and methods have already been hoisted. Variables still need
...
Interpreter.prototype.annotations = function (nodes) {
  return this.each(nodes, function (node) {
    return this.task(function () {
      ...
      if (node.constructor === ast.Let || node.constructor === ast.Def) {
        return this.annotate([ getDefinition() ], node.annotations,
          node.constructor === ast.Def ? "Def" : "Let");
      }
...
Interpreter.prototype.annotate = function (values, annotations, name) {
  return this.each(annotations, function (annNode) {
    return this.expression(annNode).then(function (annotation) {
      ...
      return rt[name + "Annotator"].match(annotation).then(function (isAnn) {
        return isAnn.ifTrue(rt.block(0, function () {
          return self.apply(annotation, "annotate" + name, [ values ]);
        }));
      });
    });
  });
...
Interpreter.prototype["let"] = function (node) {
  var generics, interpreter, name, ref;

  generics = node.generics;
  name = node.name.value;
  ref = this.scope[name];

  // If there are no generics, directly evaluate the value.
  if (generics.length === 0) {
    return this.expression(node.value).then(function (value) {
      if (!ref.isUnnamed) {
        value = rt.withGenerics(name, value, []);
      }
  ...
  // If there are generics, treat the let as a method.
  ref.func = function () {
    var genericArgs = util.slice(arguments);

    return interpreter.scoped(function () {
      return this.each(genericArgs, function (generic, i) {
        var gName = generics[i].value;

        return this.put(gName, this.newVar(gName, generic), node);
      }).then(function () {
      ...
      return rt.withGenerics(name, value, genericArgs);
    });
  };
};
...
Interpreter.prototype.unqualifiedRequest = function (node, name, pretty) {
  return this.search(name).then(function (pair) {
    ...
    if (!ref.isStatic && rec !== null && this.scope.object &&
...
Interpreter.prototype["class"] = function (node) {
  ...
  return this.scoped(object, function () {
    ...
    return this.annotate([ decl ], node.annotations, "Class");
  });
};
  ...
  return this.signature(signature, pretty).then(function (parts) {
    method = rt.method(pretty, parts,
      buildMethod(false, node.constructor === ast.Class ? function () {
        return this.objectBody(body).then(function (object) {
          return this.annotate([ object ], node.annotations, "Object")
            .then(function () {
              return object;
            });
    ...
    if (node.constructor === ast.Class) {
      method.inherit = rt.inheritor(pretty, parts,
        buildMethod(true, function (inheriting) {
          return this.objectBody(body, inheriting).then(function (object) {
            return this.annotate([ object ], node.annotations, "Object")
              .then(function () {
                return object;
              });
...
Interpreter.prototype.putDecl = function (node) {
  var decl, name;

  name = node.name.value;
  decl = this.newDecl(name, node.generics && node.generics.length || 0);

  return this.put(name, decl, node).then(function () {
    return decl;
  });
};
...
Interpreter.prototype.newDecl = function (name, generics) {
  var decl = rt.method(name, [ [ generics, 0 ] ], function () {
    if (decl.func === undefined) {
      return rt.UndefinedValue.raiseForName(rt.string(name));
    }

    return decl.func.apply(this, arguments);
  });

  decl.isStatic = true;
  decl.modulePath = this.modulePath;
...
Interpreter.prototype.reportNode = function (packet, node) {
  ...
  } else if (node.constructor === ast.Let) {
    type = "let " + node.name.value;
