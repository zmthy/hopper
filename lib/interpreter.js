...
Interpreter.prototype.expression = function (node) {
  ...
  if (constructor === ast.Self) {
    ...
    return this.resolve(this.findMeta("self"));
  }
  ...
  return this.each(nodes, function (node) {
    return this.task(function () {
      ...
      function getDefinition(name) {
        return (scope.self || scope)[name || node.name.value];
      }
      ...
      if (node.constructor === ast.Method) {
        return this.annotate([getDefinition(node.signature.name())],
...
Interpreter.prototype.decl = function (node) {
  return this.task(function () {
    ...
    return this.assert(value, rt.Pattern).then(function () {
      ...
      decl = (this.scope.self || this.scope)[node.name.value];
      type = decl.value;
  ...
  for (i = 0, l = signatures.length; i < l; i += 1) {
    ...
    for (j = 0; j < i; j += 1) {
      if (names[j] === name) {
        ...
        return rt.InvalidType.raiseForDuplicateMethodName(rt.string(name))
...
Interpreter.prototype.outer = function (node) {
  ...
  for (scope = this.scope; scope !== null; scope = scope.outer) {
    if (util.owns(scope, "self")) {
      if (i === l) {
        return this.resolve(scope.self);
      }
...
Interpreter.prototype.request = function (node, inheriting) {
  ...
  return this.task(function () {
    var l, method, overridden, self;

    if (rnode === null) {
      return this.search(name).then(function (ref) {
        if (ref === null) {
    ...
    if (rnode.constructor === ast.Super) {
      method = this.findMeta("method", true);
      overridden = this.findMeta("super", false);
      self = this.findMeta("self");
...
Interpreter.prototype.variable = function (node) {
  return this.pattern(node.pattern).then(function (pattern) {
    ...
    name = node.name.value;
    variable = (this.scope.self || this.scope)[name];

    while (!variable.isHoisting) {
      variable = variable["super"];
    }

    delete variable.isHoisting;
...
Interpreter.prototype["return"] = function (node) {
  ...
  return this.task(function () {
    ...
    var exit = this.findMeta("return", false);
...
Interpreter.prototype.search = function (name) {
  var frame, inSelf, preSelf, result, self;
  ...
  function next() {
    ...
    if (util.owns(frame, name)) {
      // Immediately return anything found in the local scope before reaching
      // the nearest self definition.
      if (preSelf) {
        return this.resolve(frame[name]);
      }

      if (inSelf) {
        return rt.InvalidRequest.raiseAmbiguousRequestForName(rt.string(name));
      }

      result = frame[name];
    } else if (util.owns(frame, "self")) {
      self = frame.self;

      if (self[name] !== undefined) {
        if (result !== null) {
          return rt.InvalidRequest
            .raiseAmbiguousRequestForName(rt.string(name));
        }
...
// Find meta information stored in scope. Some metainformation searches should
// not look past the first occurrence of self.
Interpreter.prototype.findMeta = function (name, passSelf) {
  var frame;
  ...
  for (frame = this.scope; frame !== null; frame = frame.outer) {
    ...
    if (!passSelf && util.owns(frame, "self")) {
      return null;
    }
  }
