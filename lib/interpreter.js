...
Interpreter.prototype = {
  ...
  block: function(node, callback) {
    ...
    function fromPatterns(patterns) {
      ...
      return function() {
        ...
        interpreter.clone().scoped(function(callback) {
          this.part(name, patterns.parameters, args, function (error) {
            if (error !== null) {
              callback.call(this, error);
            } else {
              this.interpret(node.body, callback);
    ...
    if (typeof node === "function") {
      ...
      this.patterns(signature, function(error, patterns) {
        if (error !== null) {
          callback.call(this, error);
        } else {
          callback.call(this, null,
            withApply(fromPatterns.call(this, patterns.parts[0])));
        }
      });
    }
  },

  assert: function(value, pattern, callback) {
    if (pattern === runtime.Unknown ||
        pattern instanceof runtime.NamedPattern &&
        pattern._pattern === runtime.Unknown) {
      callback.call(this, null);
    } else {
  ...
  branch: function(condition, ifTrue, ifFalse, callback) {
    this.apply(condition['andAlso orElse'],
      this.block(ifTrue), this.block(ifFalse), callback);
  },

  type: function(node, callback) {
    this.each(node.signatures, function(signature, callback) {
      this.patterns(signature, callback);
    }, function(error, signatures) {
      if (error !== null) {
        callback.call(this, error);
      } else {
        callback.call(this, null, new runtime.Type(signatures));
      }
    });
  },
  ...
  apply: function(receiver, method, parts, callback, inheriting) {
    var asynchronous, finished, interpreter, result;

    function applyParts(i) {
      var args, exception, result;

      args = parts[i];

      if (i < finished) {
        // If the method is multipart, it must be asynchronous.
        args.push(function(error, next) {
          if (error !== null) {
            callback.call(interpreter, error);
          } else {
            method = next;
            applyParts.call(interpreter, i + 1);
          }
        });
    ...
    // Should probably be removed (factor out the lookup in 'request').
  ...
  method: function(node, callback) {
    ...
    function buildMethod(patterns, pattern) {
      var constructor, body, finished, init, last, interpreter;
      ...
      function method() {
        ...
        // The count of processed parts.
        i = 0;

        // These need to be threaded through to 'part' below for the first call.
        args = arguments;

        interpreter.scoped(function(callback) {
          var interpreter, stack, top;

          // Save the state of the interpreter at the point where it has entered
          // the method. The previously cloned interpreter will return to its
          // previous state, ready for a separate method request.
          interpreter = this.clone();
          stack = interpreter.stack;
          top = stack[stack.length - 1];

          function part() {
            var args, callback, inheriting, l, params;

            l = arguments.length - 1;
            args = slice(arguments, 0, l);
            callback = arguments[l];
            params = patterns.parts[i].parameters;

            interpreter.part(node.name(true), params, args, function(error) {
              var exit;

              if (error !== null) {
                callback.call(this, error);
              } else if (i === finished) {
                exit = function(error, result) {
                  top._return = null;
          ...
          // This function is called by the line above, so there cannot be an
          // error. The actual callback, which is at the end of args, will be
          // processed by the 'part' function.
          part.apply(null, args);
        });
      }
    ...
    // Evaluate the patterns of the method parameters.
    this.patterns(signature, function(error, patterns) {
      var pattern;
      ...
      if (error !== null) {
        ...
        if (pattern !== null) {
          // Evaluate the return pattern.
          this.expression(pattern, function(error, pattern) {
            if (error !== null) {
              callback.call(this, error);
            } else {
          ...
          buildMethod.call(this, patterns, runtime.Unknown);
        }
      }
    });
  },

  patterns: function(signature, callback) {
    this.each(signature, function(part, callback) {
      this.each(part.parameters, function(parameter, callback) {
        var name = parameter.name;

        if (parameter.pattern === null) {
          callback.call(this, null,
            new runtime.NamedPattern(name, runtime.Unknown));
        } else {
          this.expression(parameter.pattern, function(error, pattern) {
            if (error !== null) {
              ...
              this.assert(pattern, runtime.Pattern, function(error) {
                if (error !== null) {
                  ...
                  callback.call(this, null,
                    new runtime.NamedPattern(name, pattern));
                }
              });
            }
          });
        ...
        if (error !== null) {
          ...
          callback.call(this, null,
            new runtime.SignaturePart(part.name.value, parameters));
        }
      });
    }, function(error, parts) {
      if (error !== null) {
        callback.call(this, error);
      } else if (signature.pattern === null) {
        callback.call(this, null,
          new runtime.Signature(parts, runtime.Unknown));
      } else {
        this.expression(signature.pattern, function(error, pattern) {
          if (error !== null) {
            callback.call(this, error);
          } else {
            this.assert(pattern, runtime.Pattern, function(error) {
              if (error !== null) {
                ...
                callback.call(this, null,
                  new runtime.Signature(parts, pattern));
              }
            });
          }
        });
  ...
  // Add a variable to the local scope, including pattern matching if necessary.
  part: function(name, params, args, callback) {
    ...
    if (params.length > args.length) {
      ...
      this.each(zip(params, args), function(zipped, callback) {
        var arg, param;

        param = zipped[0];
        arg = zipped[1];

        if (param.pattern !== runtime.Unknown) {
          this.assert(arg, param, function(error) {
            if (error !== null) {
              callback.call(this, error);
            } else {
              this.put(param.name(), newVar(arg), callback);
            }
          });
        } else {
          this.put(param.name(), newVar(arg), callback);
  ...
  search: function(name) {
    ...
    for (i = stack.length - 1; i >= 0; i--) {
      ...
      if (hop.call(frame, name)) {
        return frame[name];
      } else if (self === null && hop.call(frame, "self")) {
        self = frame.self();
