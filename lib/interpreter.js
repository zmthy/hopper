...
Interpreter.prototype.interpret = function (nodes) {
  function handleObject() {
    var isConfidential, method, name, outer, self;
    ...
    if (this.scope.object &&
      ...
      if (self.asString === rt.base.asString) {
        ...
        if (method !== null) {
          ...
          self.asString = rt.method("asString", 0, function () {
            return outer.asString().then(function (outer) {
              return outer.asPrimitiveString().then(function (outer) {
                return rt.string("object(" + outer + "." + name + ")");
              });
            });
          });
  ...
  for (frame = this.scope; frame !== null; frame = frame.outer) {
    if (util.owns(frame, "self")) {
      ...
      if (self[name] !== undefined) {
        return rt.lookup(self, name, true).bind(this);
      }
    }
