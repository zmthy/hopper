...
Interpreter.prototype = {
  ...
  interpret: function(nodes, callback) {
    ...
    if (nodes.length === 0) {
      callback.call(this, null, outcome);
    } else {
      self = this.self();
    ...
    this.patterns(node.parameters, function(error, patterns) {
      ...
      function apply(args, callback) {
        interpreter.clone().scoped(function(callback) {
          this.parts(name, parameters, args, function (error) {
            if (error !== null) {
              callback.call(this, error);
            } else {
              this.interpret(node.body, callback);
  ...
  method: function(node, callback) {
    ...
    function buildMethod(patterns, pattern) {
      ...
      function method() {
        var args, callback, inheriting, l, self;
        ...
        interpreter.clone().scoped(function(callback) {
          ...
          this.each(zip(signature, args), function(parts, callback) {
            this.parts(node.name(true), parts[0].parameters, parts[1],
              callback);
          }, function(error) {
            var stack, top;
  ...
  parts: function(name, sigPart, argPart, callback) {
    // TODO Handle argvars
    if (sigPart.length > argPart.length) {
      ...
      this.each(zip(sigPart, argPart), function(param, callback) {
        // TODO Perform pattern matching check.
