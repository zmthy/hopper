...
var ast, hop, path, proto, runtime, varUnknown;
...
// Strip the parentheses from method names.
...
Interpreter.prototype.module = function (key, nodes, callback) {
  this.expression(new ast.ObjectConstructor([], nodes),
    function (error, module) {
      if (error !== null) {
        callback.call(this, error);
      } else {
        this.modules[path.normalize(key)] = module;
        callback.call(this, null, module);
      }
    });
};
...
Interpreter.prototype.evaluate = function (node, callback) {
  ...
  } else if (constructor === ast.Import) {
    this["import"](node, callback);
  } else if (constructor === ast.Dialect) {
    this.dialect(node, callback);
  } else {
...
Interpreter.prototype.dialect = function (node, callback) {
  var self = this;

  this.load(node.path.value, function (error, module) {
    if (error !== null) {
...
Interpreter.prototype["import"] = function (node, callback) {
  var self = this;
  ...
  this.load(node.path.value, function (error, module) {
    ...
    if (error !== null) {
      ...
      if (name !== "_") {
        self.put(node.identifier.value, newVar(module), callback);
      } else {
        callback.call(self, null);
      }
    }
...
Interpreter.prototype.request = function (node, callback, inheriting) {
  var l, method, name, overridden, pretty, ref, rnode;
  ...
  if (rnode === null) {
    ...
    if (ref === null) {
      // Don't complain about assignment not existing when the variable that was
      // supposed to be assigned to doesn't exist in the first place.
      l = name.length - 3;
      if (name.substring(l) === " :=") {
    ...
    if (overridden === null) {
      callback.call(this,
        'No super method "' + pretty + '" to request');
    ...
    this.expression(rnode, function (error, receiver) {
      ...
      if (error !== null) {
        ...
        if (typeof method !== "function" ||
          ...
          callback.call(this,
            "No such method '" + pretty + "' in " + receiver);
        } else {
