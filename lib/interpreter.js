...
Interpreter.prototype.block = function (node) {
  var interpreter, parameters, signature;
  ...
  return rt.block([0, parameters.length], function () {
...
Interpreter.prototype.method = function (node) {
  var body, constructor, init, interpreter, last, method, name, signature;
  ...
  method = rt.newMethod(name, this.signature(signature),
...
Interpreter.prototype.signature = function (signature) {
  var i, l, part, parts;
  ...
  for (i = 0, l = signature.length; i < l; i += 1) {
    part = signature[i];
    parts.push([part.generics.length, part.parameters.length]);
  }
...
Interpreter.prototype.self = function (search) {
  var self = this.scope.self || (search && this.search("self"));
