...
Interpreter.prototype = {
    ...
    function fromPatterns(patterns) {
      ...
      return function() {
        ...
        interpreter.clone().scoped(function(callback) {
          this.part(name, patterns, args, function (error) {
            if (error !== null) {
              callback.call(this, error);
            } else {
              this.interpret(node.body, callback);
  ...
  apply: function(receiver, method, args, callback, inheriting) {
  ...
  method: function(node, callback) {
    ...
    function withPatterns(patterns, pattern) {
      var constructor, body, init, last, interpreter;

      function method() {
        var args, callback, inheriting, l;
        ...
        if (patterns.parts.length === 1) {
          args.generics = callback.generics || [];
          args = [args];
        }
        ...
        interpreter.clone().scoped(function(callback) {
          ...
          // Add the parameters of each part to the local scope.
          this.each(patterns.parts, args, function(sigPart, argPart, callback) {
            this.part(node.name(true), sigPart, argPart, callback);
          }, function(error) {
    ...
    this.each(signature.annotations, this.expression, function(error) {
      if (error !== null) {
        callback.call(this, error);
      } else {
            ...
            if (pattern !== null) {
              ...
              this.expression(pattern, function(error, pattern) {
                if (error !== null) {
                  ...
                  withPatterns.call(this, patterns, pattern);
                }
              });
            } else {
              withPatterns.call(this, patterns, runtime.Unknown);
            }
          }
        });
      }
  ...
  patterns: function(signature, callback) {
    this.each(signature, function(part, callback) {
      this.each(part.parameters, function(parameter, callback) {
        ...
        if (parameter.pattern === null || part.generics.length > 0) {
          // TODO This will cause any method with generic parameters to never
          // evaluate or test their patterns dynamically. This should be fixed
          // once the behaviour is actually defined.
          callback.call(this, null,
        ...
        var generics, gpatterns, i, l;

        if (error !== null) {
          callback.call(this, error);
        } else {
          generics = part.generics;
          gpatterns = [];

          for (i = 0, l = generics.length; i < l; i++) {
            gpatterns.push(new runtime.NamedPattern(generics[i].value,
              runtime.Pattern));
          }

          callback.call(this, null,
            new runtime.SignaturePart(part.name.value, gpatterns, patterns));
        }
      });
  ...
  part: function(name, part, args, callback) {
    // TODO Handle argvars
    var generics, patterns;

    function add(pattern, arg, callback) {
    ...
    function params(error) {
      if (error !== null) {
        ...
        this.each(patterns, args, add, callback);
      }
    }

    generics = args.generics || [];
    patterns = part.parameters;

    if (generics.length !== 0 &&
        part.generics.length > generics.length) {
      callback.call(this,
        "Not enough generic arguments for method '" + name + "'");
    } else if (part.generics.length < generics.length) {
      callback.call(this,
        "Too many generic arguments for method '" + name + "'");
    } else if (patterns.length > args.length) {
      callback.call(this,
      ...
      this.each(part.generics, function(parameter, callback) {
        this.put(parameter.name().toString(),
          newVar(runtime.Unknown), callback);
      }, params);
    } else {
      this.each(part.generics, generics, add, params);
    }
  },
