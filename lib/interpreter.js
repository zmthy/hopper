...
var Task, ast, exceptions, path, publicity, rt, types, util;
...
publicity = require("./runtime/publicity");
...
function Interpreter(moduleLoader) {
  ...
  scope = {
    ...
    asString: rt.method("asString", 0, function () {
      return rt.string("prelude");
    })
  };

  util.forProperties(types, function (name, value) {
  ...
  util.forProperties(publicity, function (name, value) {
    scope[name] = self.newVar(name, value);
  });
...
Interpreter.prototype.inheriting = function (node, inheriting) {
  ...
  return rt.InternalError
    .raiseMessage(rt.string("Unrecognised node of type " +
...
Interpreter.prototype.dialect = function (node) {
  return this.load(node.path.value).bind(this).then(function (module) {
    module.outer = this.prelude;

    if (this.scope === this.prelude) {
      this.scope = module;
    } else {
      this.scope.outer = module;
    }
  }, function (packet) {
...
Interpreter.prototype.annotations = function (values, annotations, name) {
  return this.each(annotations, function (annotation) {
    return this.expression(annotation).then(function (annotation) {
      return this.assert(annotation, types[name + "Annotator"])
        .then(function () {
...
Interpreter.prototype.object = function (node, inheriting) {
  return this.objectBody(node.body, inheriting).then(function (object) {
    return this.annotations([object], node.annotations, "Object")
      .then(function () {
        return object;
      });
...
Interpreter.prototype.decl = function (node) {
  return this.task(function () {
    ...
    return this.assert(value, rt.Pattern).then(function () {
      ...
      decl = this.search(node.name.value, false);
      type = decl.value;
      ...
      return this.annotations([decl], node.annotations, "Type");
    }).then(null, function (packet) {
...
Interpreter.prototype.boolean = function (node, inheriting) {
  ...
  if (inheriting !== undefined) {
    return this.inherit(inheriting, method, []);
  }
...
Interpreter.prototype["class"] = function (node) {
  ...
  return this.scoped(object, function () {
    ...
    return this.put(name, def, node).then(function () {
      return this.annotations([def], node.annotations, "Class");
    });
  });
};
...
Interpreter.prototype.method = function (node) {
  ...
  return this.signature(signature, pretty).then(function (parts) {
    method = rt.method(pretty, parts,
      buildMethod(node.constructor === ast.Class ? function () {
    ...
    if (node.constructor === ast.Class) {
      method.inherit = rt.method(pretty, parts,
        buildMethod(function (inheriting) {
    ...
    return this.put(pretty, method, node).then(function () {
      if (node.constructor === ast.Method) {
        return this.annotations([method], node.annotations, "Method");
      }
    });
  });
};
...
Interpreter.prototype.variable = function (node) {
  return this.pattern(node.pattern).then(function (pattern) {
    ...
    return this.task(function () {
      ...
      return this.putVariable(node, pattern);
    }).then(function (variable) {
      return this.task(function () {
        if (node.constructor === ast.Var) {
          return this.annotations([variable,
            this.scope[name + ":="]], node.annotations, "Var");
        }

        return this.annotations([variable], node.annotations, "Def");
      }).then(function () {
        if (node.value !== null) {
...
Interpreter.prototype.put = function (pretty, method, node) {
  var existing, name, self, sup, top;
