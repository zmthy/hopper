...
Interpreter.prototype.block = function (node) {
  ...
  function subtype(direction) {
    return function (other) {
      return rt.Pattern.assert(other).then(function () {
        return interpreter.pattern(patternNode);
      }).then(function (pattern) {
        var store;

        if (other instanceof rt.primitives.Block) {
          store = other.object;

          if (typeof store === "object" &&
              typeof store.pattern === "object") {
            return interpreter.pattern(other.object.pattern)
              .then(function (otherPattern) {
                return pattern[direction](otherPattern);
              });
          }
        ...
        return pattern[direction](other);
      });
    };
  }
  ...
  if (parameters.length === 1) {
    ...
    if (patternNode !== null) {
      block.object = { "pattern": patternNode };
...
Interpreter.prototype.parts = function (msig, rsig, node) {
  return this.each(msig.parts, rsig, function (mpart, rpart) {
    ...
    return this.each(msig.clauses, function (clause) {
      return this.expression(clause).then(function (isValid) {
        return rt.Boolean.cast(isValid);
      }).then(function (isValid) {
        return isValid.orElse(rt.block(0, function () {
          return rt.AssertionFailure
            .raiseForClause_ofMethod([ rt.string(clause.toString()) ],
              [ rt.string(msig.name()) ]);
        }));
      });
    }).then(function () {
