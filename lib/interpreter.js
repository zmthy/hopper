...
var ast, hop, runtime, step, undefined;
...
Interpreter.prototype = {
  ...
  evaluate: function(node, callback) {
    ...
    } else if (node.constructor === ast.Def || node.constructor === ast.Var) {
  ...
  expression: function(node, callback) {
    if (node.constructor === ast.Request) {
      this.request(node, callback);
    } else if (node.constructor === ast.ObjectConstructor) {
      this.object(node, callback);
    } else if (node.constructor === ast.Block) {
      this.block(node, callback);
    } else if (node.constructor === ast.StringLiteral) {
      this.string(node, callback);
    } else if (node.constructor === ast.NumberLiteral) {
      this.number(node, callback);
    } else {
      callback.call(this, "Unrecognised node of type " + node.constructor);
    }
  },

  object: function(node, callback) {
  ...
  request: function(node, callback) {
        ...
        if (error) {
          ...
          if (method.asynchronous) {
            ...
            args.push(this.asynchronous ? function(error, result) {
              callback.call(self, error, outcome = result);
            } : function() {
          ...
          if (this.asynchronous && method.asynchronous) {
            ...
            try {
              callback.call(this, null, method.apply(receiver, args));
            } catch(error) {
              callback.call(this, error);
            }
          }
    ...
    if (receiver === null) {
      ...
      if (ref === null) {
        ...
        callback.call(this, "'" + node.name(true) + "' is not defined");
      } else {
  ...
  self: function() {
    ...
    return stack[stack.length - 1].self;
  },

  put: function(name, method, callback) {
    var self, stack, top;
    ...
    if (hop.call(top, name)) {
      callback.call(this, "'" + node.name(true) + "' is already defined");
    } else {
      top[name] = method;
...
// Create a new variable accessor, which stores the value it is accessing as a
// property.
...
// Yield control of the thread, and then resume as soon as possible with the
// given action.
step =
  ...
  function(action) { setTimeout(action, 0); };
