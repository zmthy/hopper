  ...
  return this.load(name).bind(this).then(function (module) {
    return this.task(function () {
      if (typeof module.checker === "function") {
        return module.checker(rt.list(nodes));
      }
    }).then(function () {
      var scope = {
        outer: null,
      ...
      if (packet.object.isHandled) {
        throw packet;
      }

      return new Task(this, function (resolve) {
        var self = this;
        ...
        rt.CheckerFailure.match(packet).then(function (match) {
          return match.ifTrue(rt.block(0, function () {
            var location = packet.object.stackTrace[0].location.module;

            packet.object.stackTrace = [];
            packet.object.isHandled = true;

            if (location !== name) {
              return stop();
            }

            return packet.nodeOrIfAbsent(rt.block(0, function () {
              return stop();
            })).then(function (node) {
              return self.reportNode(packet, node).then(null, stop);
            });
          }));
        }).callback(stop);
      }).then(function () {
...
Interpreter.prototype.search = function (name) {
  ...
  function next() {
    ...
    if (util.owns(frame, name)) {
      ...
      if (preSelf) {
        // If we've hit self, return the value in that, as it may be overridden.
        if (util.owns(frame, "self")) {
          return this.resolve(frame.self[name]);
        }
