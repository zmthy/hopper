...
Interpreter.prototype.interpret = function (nodes) {
  ...
  if (Date.now() - this.timer > 10) {
    return new Promise((resolve, reject) => {
      setImmediate(() => {
        self.timer = Date.now();

        try {
          self.interpretNodes(nodes).then(resolve, reject);
        } catch (error) {
          reject(error);
        }
      });
  ...
  return this.interpretNodes(nodes);
};

Interpreter.prototype.interpretNodes = function (nodes) {
  var handleObject = () => {
      ...
      if (context.asString === rt.base.asString) {
        ...
        if (method !== null) {
          ...
          context.asString = rt.method("asString", 0, function () {
            return outer.asString().then((string) => {
              return string.asPrimitiveString().then((pstring) => {
...
Interpreter.prototype.module = function (nodes, key) {
  ...
  return interpreter.objectBody(nodes, module).then(() => {
  ...
  if (Date.now() - this.timer > 10) {
    return new Promise((resolve, reject) => {
      setImmediate(() => {
        ...
        try {
          self.evaluateNode(node).then(resolve, reject);
        } catch (error) {
          reject(error);
        }
      });
...
Interpreter.prototype.expression = function (node) {
  ...
  if (constructor === ast.Self) {
    if (this.scope.object) {
      return rt.IncompleteObject.raiseForSelf().then(null, (packet) => {
...
Interpreter.prototype.check = function (nodes) {
  ...
  if (nodes.length > 0 && nodes[0].constructor === ast.Dialect) {
    ...
    return this.dialect(nodes[0], nodes).then(() => {
      return nodes;
    }, function (packet) {
...
Interpreter.prototype.dialect = function (node, nodes) {
  ...
  return this.load(name).then((module) => {
    return this.task(() => {
...
Interpreter.prototype["import"] = function (node) {
  return this.load(node.path.value).then((module) => {
...
Interpreter.prototype.annotate = function (values, annotations, name) {
  return this.each(annotations, (annNode) => {
    return this.expression(annNode).then((annotation) => {
...
Interpreter.prototype.object = function (node, inheriting) {
  return this.objectBody(node.body, inheriting).then((object) => {
...
Interpreter.prototype.block = function (node) {
  ...
  block = rt.block([0, parameters.length], function () {
    ...
    return interpreter.clone().scoped(function () {
      return this.parts(signature, args, node).then(() => {
  ...
  if (parameters.length === 1) {
    ...
    if (patternNode !== null) {
      block.match = rt.method("match()", 1, function (object) {
        return interpreter.pattern(patternNode).then((pattern) => {
          return pattern.match(object).then((match) => {
            return match.andAlso(rt.block(0, function () {
              ...
              return interpreter.clone().scoped(function () {
                ...
                return this.task(() => {
                  ...
                  return this.interpret(node.body).then((result) => {
...
Interpreter.prototype.decls = function (nodes) {
  return this.each(nodes, (node) => {
    ...
    return this.each(declNodes, (node) => {
      return this.decl(node);
    }).then((decls) => {
      return this.each(declNodes, decls, (node, pattern) => {
        return this.putDecl(node, pattern);
      });
    }).then(() => {
      return this.each(declNodes, (node) => {
  ...
  return this.task(() => {
    if (node.generics.length !== 0) {
      ...
      return this.scoped(function () {
        return this.each(node.generics, (parameter) => {
...
Interpreter.prototype.putDecl = function (node, pattern) {
  ...
  return this.assert(pattern, rt.Pattern).then(() => {
...
Interpreter.prototype.type = function (node, decl) {
  var i, j, l, name, names, nsignatures, report, tsignatures;

  report = (packet) => this.report(packet, "type", null, node);
  ...
  return this.task(() => {
    ...
    return this.each(node.parts, (part) => {
      ...
      return this.each(part.generics, (param) => {
        ...
        return this.each(part.arguments, (node) => this.expression(node))
          .then((args) => {
      ...
      }).then(null, rt.handleInternalError).then(null, (packet) => {
...
Interpreter.prototype.unqualifiedRequest = function (node, name, pretty) {
  return this.search(name).then((pair) => {
    var l, rec, ref;
    ...
    if (pair === null) {
      ...
      if (name.substring(l) === ":=") {
        ...
        return this.search(name.substring(0, l)).then((found) => {
          if (found === null) {
...
Interpreter.prototype.qualifiedRequest = function (node, name, pretty) {
  ...
  return this.expression(rnode).then((receiver) => {
  ...
  function buildMethod(isInherits, func) {
    return function (inheriting) {
      ...
      return clone.scoped(function () {
        return new Promise((resolve, reject) => {
          this.parts(signature, argParts, node).then((pattern) => {
            var exit, top;
            ...
            exit = (value) => {
              ...
              this.assert(value, pattern).then(() => {
                resolve(value);
              }, (packet) => {
            ...
            return func.call(this, inheriting).then(exit, reject);
          }, reject);
        });
      });
    };
  ...
  return this.signature(signature, pretty).then((parts) => {
    ...
    if (node.constructor === ast.Class) {
      ...
      if (constructor === ast.ObjectConstructor) {
        ...
        method.inherit = rt.inheritor(pretty, parts,
          buildMethod(true, function (inheriting) {
            return this.interpret(init).then(() => {
...
Interpreter.prototype.signature = function (signature, pretty) {
  var hasVarArg, i, j, k, l, param, params, part, parts, report;

  report = (packet) => this.report(packet, "method", null, part);
...
Interpreter.prototype.parts = function (msig, rsig, node) {
  return this.each(msig.parts, rsig, (mpart, rpart) => {
...
Interpreter.prototype.generics = function (mgens, rgens, node) {
  return this.each(mgens, rgens, (mgen, rgen) => {
    if (mgen.value !== "_") {
...
Interpreter.prototype.parameters = function (params, args, node) {
  return this.each(params, (param, i) => {
    var varArgSize = args.length - params.length + 1;
    if (param.isVarArg) {
      args.splice(i, 0, rt.list(args.splice(i, varArgSize)));
    }
  }).then(() => {
    return this.patterns(params).then((patterns) => {
      return this.each(params, (param) => {
...
Interpreter.prototype.patterns = function (parameters) {
  return this.each(parameters, (parameter) => {
    var name = parameter.name.value;

    return this.pattern(parameter.pattern).then((pattern) => {
...
Interpreter.prototype.pattern = function (expression) {
  ...
  return this.expression(expression).then((pattern) => {
...
Interpreter.prototype.args = function (names, patterns, args, node) {
  return this.each(names, patterns, args, (name, pattern, arg) => {
    return this.assert(arg, pattern).then(() => {
      if (name !== "_") {
...
Interpreter.prototype.variable = function (node) {
  return this.pattern(node.pattern).then((pattern) => {
    var name, variable;
    ...
    if (node.value !== null) {
      return this.expression(node.value).then((value) => {
        return this.assert(value, pattern).then(() => {
...
Interpreter.prototype.putVariable = function (node, pattern) {
  ...
  return this.put(name, variable, node).then(() => {
    var self, setter;
    ...
    if (node.constructor === ast.Var) {
      ...
      setter = rt.method(name + " :=", 1, function (value) {
        return self.assert(value, variable.pattern).then(() => {
          variable.value = value;
          return rt.done;
        });
      });
...
Interpreter.prototype.inherits = function (node) {
  ...
  return this.inheriting(node.request, context).then((value) => {
    delete this.scope.object;
    return value;
  }, (packet) => {
...
Interpreter.prototype.scoped = function (context, action) {
  ...
  this.push(context);
  return this.task(action).then((value) => {
    this.pop();
    return value;
  }, (reason) => {
    this.pop();
  ...
  for (frame = this.scope; frame !== null; frame = frame.outer) {
    if (util.owns(frame, "self")) {
      ...
      if (context[name] !== undefined) {
        return rt.lookup(context, name, true).then(pair);
      }
    }
