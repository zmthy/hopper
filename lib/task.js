...
Task.prototype.then = function (onFulfilled, onRejected) {
  return then(this, function (res, reject, fresh) {
    ...
    deferred = util.once(function (force) {
      ...
      function fulfiller() {
        return completion(task, fresh, onFulfilled, resolve, resolve, reject);
      }

      function rejecter() {
        return completion(task, fresh, onRejected, reject, resolve, reject);
      }
      ...
      if (task.isPending) {
        task.onFulfilled.push(fulfiller());
        task.onRejected.push(rejecter());
      } else {
        if (util.owns(task, "value")) {
          fulfiller()(task.value);
        } else {
          rejecter()(task.reason);
        }
      }
    });
