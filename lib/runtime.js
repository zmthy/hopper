...
function inherit(inheriting, method, args) {
  if (typeof method.inherit !== "function") {
    throw 'Method "' + method.identifier +
      '" does not end in an object constructor"';
  }
...
function newMethod(name, partCounts, func) {
  ...
  function method() {
    ...
    if (argsLength < partsLength) {
      throw 'Not enough parts for method "' + name + '"';
    }

    if (argsLength > partsLength) {
      throw 'Too many parts for method "' + name + '"';
    }
...
function block(parameters, apply) {
  var object = defs.object();

  object.apply = newMethod("apply" + ((typeof parameters === "number" ?
      parameters : parameters[1]) === 0 ? "" : "()"), [parameters], apply);
