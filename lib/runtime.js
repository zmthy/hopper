...
function lookup(receiver, pretty, fromSelf) {
  ...
  if (typeof func !== "function" || !func.isGraceMethod) {
    if (typeof func === "function") {
      ...
      func = method("at()", 1, function (name) {
        var self = this;

        return defs.asString(name).then(function (name) {
          return fromPrimitive(self[name]);
        });
      });
    } else if (pretty === "at() put()") {
      func = method("at() put()", [1, 1], function (name, value) {
        var self = this;
...
function inherit(receiver, method, inheriting, args) {
  ...
  return call(receiver, method.inherit, args);
}
