...
function method(name, partCounts, func) {
  ...
  body = function () {
    ...
    return task.each(partCounts, argParts, (partCount, argPart) => {
      ...
      if (util.isArray(argPart.generics) && argPart.generics.length !== 0) {
        ...
        return task.each(argPart.generics, (generic) => {
      ...
      return value;
    }, handleInternalError).then(null, (packet) => {
...
function asPrimitive(object) {
  return Promise.resolve(typeof object.asPrimitive === "function" ?
    ...
    if (typeof func === "function") {
      if (!func.isGraceMethod) {
        ...
        func = method(pretty, function () {
          ...
          return task.each(util.slice(arguments), (object) => {
            return asPrimitive(object);
          }).then((args) => {
      ...
      func = method("at()", 1, function (index) {
        ...
        return defs.asString(index).then((primIndex) => {
      ...
      func = method("at() put()", [1, 1], function (index, value) {
        ...
        return defs.asString(index).then((primIndex) => {
          return asPrimitive(value).then((primValue) => {
...
function call(receiver, meth, args) {
  try {
    return Promise.resolve(meth.apply(receiver, args))
      .then(null, handleInternalError);
  } catch (reason) {
    return Promise.reject(handleInternalError(reason));
  }
}
...
function apply(receiver, meth, args) {
  if (typeof meth === "string") {
    return lookup(receiver, meth).then((foundMethod) => {
