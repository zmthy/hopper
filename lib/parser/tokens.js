...
Token.prototype.validate = function (lexer) {
  if (this.value.length === 0) {
    lexer.error("Empty token of type " + this.type);
  }
...
NumberLiteral.prototype.validate = function (lexer) {
  ...
  if (value[0] === "0" &&
    ...
    lexer.error("Leading zero on " + this);
  }
  ...
  if (x !== null && x.index !== value.length - 1) {
    ...
    if (base < 2 || base > 36 || isNaN(base)) {
      lexer.error(base + " is not a valid numerical base");
    }

    for (i = x.index + 1, l = value.length; i < l; i += 1) {
      if (isNaN(parseInt(value[i], base))) {

        lexer.error("'" + value[i] + "' is not a valid digit in base " + base);
      }
    }
  } else {
    ...
    if (/[eExX\+\-]/.test(last)) {
      lexer.error("Dangling modifier on " + this);
    }

    if (last === ".") {
      lexer.error("Dangling decimal point on " + this);
    }
  }
...
StringLiteral.prototype.toString = function () {
  return 'the string literal "' + unicode.escape(this.value) + '"';
};
