...
Lexer.prototype.nextToken = function (interpolating) {
  var c, e, escaped, dot, i, l, lexer, spaced, text, token, value, x;
  ...
  function futureControlError(message, offending) {
    if (token.validate === tokens.StringLiteral.prototype.validate) {
      token.validate = function (lexer) {
        lexer.error(message + " «" + offending + "» in " + this);
      };
    }
  }
  ...
  while (i < l) {
    ...
    if (token.constructor === tokens.Identifier) {
      ...
      if (c === "\n") {
        ...
        token.value += "\ufffd";
        futureControlError("Invalid control character", unicode.escape(c));
      } else if (escaped) {
        if (/["\\\\{}]/.test(c)) { //"]) {
          token.value += c;
        } else if (c === "n") {
