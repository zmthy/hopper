...
util.inherits(Request, Node);

Request.prototype.name = buildName;

Request.prototype.isBinaryOperator = function () {
  var name = this.parts[0].name;

  return name.isOperator && name.value.substring(0, 6) !== "prefix";
};

Request.prototype.isPrefixOperator = function () {
  var name = this.parts[0].name;
...
util.inherits(BooleanLiteral, Node);
...
util.inherits(NumberLiteral, Node);
...
util.inherits(StringLiteral, Node);
...
util.inherits(ObjectConstructor, Node);
