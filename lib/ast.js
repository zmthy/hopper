...
// new Let(name : Identifier, generics : [Identifier],
//     annotations : [Expression], value : Expression)
//   A static declaration.
function Let(name, generics, annotations, value, token) {
  Node.call(this, token);
...
util.inherits(Let, Node);

Let.prototype.accept = function (visitor) {
  var self = this;

  return visitor.visitLet(self).then(function () {
    return self.name.accept(visitor);
  }).then(function () {
    return acceptAll(self.generics, visitor);
  }).then(function () {
...
Let.prototype.toString = function () {
  return "let " + this.name + commas("<", this.generics, ">") +
    commas(" is ", this.annotations, "") + " = " + this.value;
};
