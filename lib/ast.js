...
function name(pretty) {
  ...
  if (signature.length === 1) {
    ...
    if (part.parameters.length === 0 ||
        lexer.isSymbol(value[value.length - 1])) {
      return value;
    }
  }
...
// commas(left : String, list : List, right : String) : String
...
//     generics : List<Expression>, parameters : List<Expression>)
...
//     annotations : List<Expression>, value : Expression)
//   A definition declaration.
function Def(name, pattern, annotations, value) {
  this.name = name;
...
function Var(name, pattern, annotations, value) {
  this.name = name;
  this.pattern = pattern;
  this.annotations = annotations;
  this.value = value;
...
// new ObjectConstructor(annotations : List<Expression>,
//     body : List<Statement | Method>)
//   An object constructor.
function ObjectConstructor(annotations, body) {
  this.annotations = annotations;
  this.body = body;
}
...
Method.prototype.name = name;

Method.prototype.toString = function () {
  var annotations, body, braceSep, pattern, signature;
  ...
  return "method " + signature.join(" ") +
    ...
    commas(" is ", annotations, "") + " {" + braceSep +
    body.join("\n") + braceSep + "}";
};

// new SignaturePart(name : Identifier,
//     generics : List<Identifier>, parameters : List<Parameter>)
...
Type.prototype.nameOf = function (i, pretty) {
  return name.call({ signature: this.signatures[i] }, pretty);
};
...
// new TypeDeclaration(name : Identifier, generics : List<Type>,
//     annotations : List<Expression>, value : Type)
//   A new type declaration.
function TypeDeclaration(name, generics, annotations, value) {
  this.name = name;
  this.generics = generics;
  this.annotations = annotations;
  this.value = value;
